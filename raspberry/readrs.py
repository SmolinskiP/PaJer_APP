...
def Success_Error_Window(success_errow_window_type):
    ...
    if success_errow_window_type == 42:
        ...
        color = 'green'
    Success_Error_Window_Label = tk.Label(Success_Error_Window, text=string, fg=color, font=(None, 25, 'bold'))
    Success_Error_Window_Label.place(x=250, y=150, anchor="center")
...
def Button_Window():
    ...
    enter_btn = tk.Button(Window, text="Wejscie", width=12, height=4, font = fnt.Font(size = 21), bg="#26580F", activebackground="#86DC3D", command= lambda: check_entry(employee_id, 1)).grid(row=1, column=0)
    exit_btn = tk.Button(Window, text="Wyjscie", width=12, height=4, font = fnt.Font(size = 21), bg="#D1001F", activebackground="#EE6B6E", command= lambda: check_entry(employee_id, 2)).grid(row=1, column=1)
    pause_start_btn = tk.Button(Window, text="Przerwa", width=12, height=4, font = fnt.Font(size = 21), bg="#C5C6D0", activebackground="#D9DDDC", command= lambda: check_entry(employee_id, 3)).grid(row=2, column=0)
    pause_end_btn = tk.Button(Window, text="Koniec przerwy", width=12, height=4, font = fnt.Font(size = 21), bg="#808B96", activebackground="#D9DDDC", command= lambda: check_entry(employee_id, 4)).grid(row=2, column=1)
